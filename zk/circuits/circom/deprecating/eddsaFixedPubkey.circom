...
// Calculate second part of the right side:  right2 = h*8*A
    ...
    component h8a = EscalarMulFix(254, A8);
    for (i=0; i<254; i++) {
        h8a.e[i] <== h2bits.out[i];
    }
...
// Compute the right side: right =  R8 + right2
    ...
    addRight.x1 <== R8x;
    addRight.y1 <== R8y;
    addRight.x2 <== h8a.out[0];
    addRight.y2 <== h8a.out[1];
...
// Calculate left side of equation left = S*B8
    ...
    component sb8 = EscalarMulFix(253, BASE8);
    for (i=0; i<253; i++) {
        sb8.e[i] <== snum2bits.out[i];
    }
