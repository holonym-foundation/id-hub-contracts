...
if (typeof window === "undefined") {
	...
	ZK_DIR =
		process?.env?.ZKESCROW_CUSTOM_ZK_DIR ||
...
async function processInput(
	...
	const msgsAsPoints: Array<Array<string>> = msgsAsPointObjects.map((obj) => [
		obj.x,
		obj.y,
	]);
	...
	const prfOuts: Array<string> = prfData.map((d) =>
		BigInt(d.prfOut).toString(),
	);
	const prfOutAsPointObjects: Array<Point> = await Promise.all(
		prfOuts.map((p) => Utils.msgToPoint(p.toString())),
	);
	const prfOutAsPoints: Array<Array<string>> = prfOutAsPointObjects.map(
		(obj) => [obj.x, obj.y],
	);
 ...
 * @param msgToEncrypt - an array of messages that need to be encrypted.
 *    These messages are base10-strings of numbers
 *       less than 21888242871839275222246405745257275088614511777268538073601725287587578984328 << 10,
 *        where << is the bitshift operator.
