...
contract IsUSResidentV2 is PaidProof {
    ...
    // This does not check the hashbrown/masala/nullifer from the legacy contract, so be careful
    function proofIsValidNonLegacy(Proof calldata proof, uint[6] calldata input) public view returns (bool isValid) {
    ...
    function prove(Proof calldata proof, uint[6] calldata input) public {
        require(proofIsValidNonLegacy(proof, input));
