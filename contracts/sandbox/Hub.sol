...
// A stripped-down version of Hub, intended for use on testnet for development purposes.
...
contract SandboxHub is Ownable, ERC721URIStorage {
    ...
    constructor() ERC721("Holonym V3", "H3") {}
    ...
    /// `sbtReciever` is the address to recieve the SBT. 
    /// `expiration` is an expiration date
    ...
    function setSBT(
        ...
        if(nullifier != 0) {
            require(!usedNullifiers[nullifier], "this has already been proven");
            usedNullifiers[nullifier] = true;
        }

        bytes32 identifier = getIdentifier(sbtReceiver, circuitId);
