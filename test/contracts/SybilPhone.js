...
const createSRProof= async ({tree, actionId, hashbrowns, address, addr, secret, customFields, iat, scope}) => {
    const leaf = poseidon([addr, secret, customFields[0], 0, iat, scope]);
    ...
    const proofArgs = `${[
        ...
        await exec(`zokrates compute-witness -a ${proofArgs} -i zk/compiled/sybilPhone.out -o tmp.witness`);
        await exec(`zokrates generate-proof -i zk/compiled/sybilPhone.out -w tmp.witness -p zk/pvkeys/sybilPhone.proving.key -j tmp.proof.json`);
...
describe.only("SybilResistance Phone", function () {
    before(async function() {
        ...
        this.sr = await (await ethers.getContractFactory("SybilPhone"))
        ...
        this.hashbrowns = poseidon([this.actionId, this.leaves.correct.newLeaf.inputs.secret]);
        this.wrongIssuerHashbrowns = poseidon([this.actionId, this.leaves.wrongIssuer.newLeaf.inputs.secret]);
    ...
    describe("Verifier works:", function() {
        ...
        it("Invalid proof doesn't work: root", async function() {
            ...
            const roots_ = await (await ethers.getContractFactory("Roots"))
                .deploy();
            const sr = await (await ethers.getContractFactory("SybilPhone"))
